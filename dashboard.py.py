...
class SignupPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        header = tk.Frame(self, bg="#4E06A7")
        header.pack(fill="x")
        tk.Label(header, text="📝 MIST Dashboard", font=("Helvetica", 24, "bold"), bg="#4E06A7", fg="white").pack(pady=20)
        ...
        section_combo = ttk.Combobox(form_frame, textvariable=self.section_var, values=["Select", "Computer Science and Engineering", "Artificial Intelligence and Machine Learning", "Other"], state="readonly")
        ...
        self.description_text = scrolledtext.ScrolledText(form_frame, height=6, wrap="word")
        ...
        ttk.Button(button_frame, text="SUBMIT", command=self.signup).pack(side="left", padx=5)
        
        ttk.Button(button_frame, text="RETURN TO LOGIN", command=self.back_to_home).pack(side="right", padx=5)
    ...
    def signup(self):
        ...
        if not all([firstname, lastname, section,]):
